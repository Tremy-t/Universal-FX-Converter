...
btn.onclick = () => {
    ...
    if (userEnteredAmount < 1 || isNaN(userEnteredAmount)) {
        numberInputField.style.borderColor = "red";
        resultTag.style.color = "red";
        resultTag.textContent = "Error: Only numeric values greater than 0 are allowed.";
    } else {
        numberInputField.style.borderColor = "var(--surface-border)";
        resultTag.style.color = "var(--primary)";
...
async function fetchData(url) {
    try {
        ...
        resultTag.style.color = "red";
        resultTag.textContent = `Fetch Error: ${error.message}`;
        throw error;
    }
}
